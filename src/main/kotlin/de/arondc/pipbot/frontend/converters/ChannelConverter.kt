...
class ChannelEntityToDTOConverter : Converter<ChannelEntity, ChannelDTO> {
    override fun convert(source: ChannelEntity): ChannelDTO {
        return ChannelDTO(source.id, source.name, source.language, source.active, source.shoutoutOnRaid, source.automatedShoutoutChannels)
    }
}

class ChannelDTOToEntityConverter : Converter<ChannelDTO, ChannelEntity> {
    override fun convert(source: ChannelDTO): ChannelEntity {
        return ChannelEntity(source.name, source.language, source.shoutOutOnRaidMode, source.shoutoutChannels, source.active, source.id)
    }
}